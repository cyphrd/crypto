(function(){
	...
	Crypto.register('SHA256', '1.2', {
		raw: function(s){
			s = Crypto.encode('UTF8', s);
			return Crypto.decode('Endian', binb_sha256(Crypto.encode('Endian', s), s.length * 8));
		},
		...
		raw_hmac: function(key, data){
			...
			var hash = binb_sha256(ipad.concat(Crypto.encode('Endian', data)), 512 + data.length * 8);
			return Crypto.decode('Endian', binb_sha256(opad.concat(hash), 512 + 256));
		},
		...
		Tests: {
			Truism: function(){
				return Crypto.SHA256.hex("abc").toLowerCase() ==
