...
Crypto.Entropy = new Class.Singleton({

	/*	Start by declaring static storage and initialize
		the entropy vector from the time we come through
		here. */
	entropyData: [],  	    			// Collected entropy data
	edlen: 0,	 	    	    	    // Keyboard array data length

	initialize: function(){
		this.addTime();	    	    	    // Start entropy collection with page load time
		this.capture();   	    	    	   	// Roll milliseconds into initial entropy
	},

	//	Add a byte to the entropy vector
	addByte: function(b) {
		this.entropyData[this.edlen++] = b;
	},
	...
	capture: function() {
		this.addByte(Math.floor((((new Date).getMilliseconds()) * 255) / 999));
	},
	...
	mouseMotion: function(maxsamp){
		if (this.mouseMotionCollect <= 0) {
		    this.mouseMotionCollect = maxsamp;
		    window.addEvent('mousemove', this.mouseMove.bindWithEvent(this));
		}
	},
	...
	mouseEntropyTime: 0,	    // Delay counter for mouse entropy collection

	mouseMove: function(event) {
		console.log('event', event);
		var e = event.event;
		if (this.mouseMotionCollect > 0) {
		    if (this.mouseEntropyTime-- <= 0) {
				this.addByte(e.screenX & 0xFF);
				this.addByte(e.screenY & 0xFF);
				this.capture();
				this.mouseMotionCollect--;
				this.mouseEntropyTime = (this.entropyData[edlen - 3] ^ this.entropyData[edlen - 2] ^ this.entropyData[edlen - 1]) % 19;
			}

		    if (this.mouseMotionCollect <= 0) {
		    	window.removeEvent(this.mouseMove.bindWithEvent(this));
		    }
		}
	},
	...
	createKey: function() {
		...
		md5_init();
		for (i = 0; i < this.edlen; i += 2) {
		    md5_update(this.entropyData[i]);
		}
		md5_finish();
			for (i = 0; i < 16; i++) {
		    k[i] = digestBits[i];
