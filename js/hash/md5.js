...
Crypto.register('MD5', '1.0', {
	raw: function(s){
		s = Crypto.encode('UTF8', s);
		return Crypto.Endian.md5_decode(binl_md5(Crypto.Endian.md5_encode(s), s.length * 8));
	},
	...
	raw_hmac: function(key, data){
		...
		var bkey = Crypto.Endian.md5_encode(key);
		if(bkey.length > 16)
		...
		var hash = binl_md5(ipad.concat(Crypto.Endian.md5_encode(data)), 512 + data.length * 8);
		return Crypto.Endian.md5_decode(binl_md5(opad.concat(hash), 512 + 128));
	},
	...
	Tests: {
		Truism: function(){
			return Crypto.MD5.hex("abc").toLowerCase() == "900150983cd24fb0d6963f7d28e17f72";
		}
	}
});
