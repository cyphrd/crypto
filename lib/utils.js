...
// 	return '' +
// 		sha512.hex(result.substr(0, result.length/2)) +
// 		sha512.hex(result.substr(result.length/2, result.length));
// }
 ...
 */
function hashx(str, nohex, full)
{
	var s = sha512.hex(str),
		ss = '';
	...
	return nohex ? hex.decode(ss) : ss;
}

function hash(str)
{
	return hashx(str + hashx(str, true, true));
}
