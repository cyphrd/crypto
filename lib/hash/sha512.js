...
// don't recommend you use this, but you can, it goes to RAW.
var sha512 = function(s, utf8safe) {
	if (utf8safe === true) s = utf8.enc(s);
	return endian.dec(binb_sha512(endian.enc(s), s.length * 8));
};

sha512.hex = function (s, utf8safe) {
	return hex.enc(sha512(s, utf8safe));
};

sha512.b64 = function (s, utf8safe) {
	return b64.enc(sha512(s, utf8safe));
};

// Calculate the HMAC-SHA-512 of a key and some data (raw strings)
sha512.hmac = function (key, data, utf8safe) {
	if (utf8safe === true) {
		key = utf8.enc(key);
		data = utf8.enc(data);
	}

	var bkey = endian.enc(key);
	if (bkey.length > 32) {
	...
	var hash = binb_sha512(ipad.concat(endian.enc(data)), 1024 + data.length * 8);
	return endian.dec(binb_sha512(opad.concat(hash), 1024 + 512));
};

sha512.hmac.hex = function (key, data, utf8safe) {
	return hex.enc(sha512.hmac(key, data, utf8safe));
};

sha512.hmac.b64 = function (key, data, utf8safe) {
	return b64.enc(sha512.hmac(key, data, utf8safe));
};
